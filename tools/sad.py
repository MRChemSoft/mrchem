...
def uncontract_dfunctions(basis_name, atomic_symbol):
    ...
    bas = bse.get_basis(name=basis_name, elements=[atomic_symbol], fmt='nwchem', header=True, uncontract_segmented=True)
    
    return bas

def get_basis_in_mrchem_format(basis_name, atomic_symbol):
    bas = bse.convert_formatted_basis_str(uncontract_dfunctions(basis_name=basis_name, atomic_symbol=atomic_symbol), 'nwchem', 'dalton')
    ...
    b = []
    b.append(f'Gaussian basis {basis_name}\n')
    b.append('1\n')
...
def sad(basis_name, atomic_symbol):
    """Compute density matrix for passed atom, and write density and basis set to file."""
    # Build molecule input
    basis = uncontract_dfunctions(basis_name=basis_name, atomic_symbol=atomic_symbol)
    ...
    basis_mrc = get_basis_in_mrchem_format(basis_name, atomic_symbol)
...
if __name__ == '__main__':
    ...
    print(f'{f"LDA/{args.basis} energy for":15}: {args.element}')
