...
void compute_local_X(double prec, Density &rho, OrbitalVector &Phi, OrbitalVector &X, DensityType spin, bool tda = false);
 ...
 *
 */
void density::compute(double prec, Density &rho, OrbitalVector &Phi, OrbitalVector &X, OrbitalVector &Y, DensityType spin, bool tda) {
    int N_el = orbital::get_electron_number(Phi);
    ...
    Density rho_loc(false);
    if (&X == &Y) {
        density::compute_local_X(rel_prec, rho_loc, Phi, X, spin, tda);
    } else {
...
/** @brief Compute local density as the sum of own (MPI) orbitals
 */
void density::compute_local(double prec, Density &rho, OrbitalVector &Phi, OrbitalVector &X, OrbitalVector &Y, DensityType spin, bool tda) {
    if (&X == &Y) {
        density::compute_local_X(prec, rho, Phi, X, spin, tda);
    } else {
        density::compute_local_XY(prec, rho, Phi, X, Y, spin);
    }
}

void density::compute_local_X(double prec, Density &rho, OrbitalVector &Phi, OrbitalVector &X, DensityType spin, bool tda) {
    int N_el = orbital::get_electron_number(Phi);
