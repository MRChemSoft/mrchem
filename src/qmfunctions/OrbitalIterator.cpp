...
bool OrbitalIterator::next( bool symmetric, int max_recv ) {
    ...
    if (max_recv<0) maxget = this->orbitals->size(); //unlimited
	...
	if (    (this->sent_counter >= send_size or !IamSender)
	    and (this->received_counter >= recv_size or !IamReceiver)) {
