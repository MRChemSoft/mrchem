...
public:
    explicit SCFEnergy(double kin = 0.0, double nn = 0.0,
                       ...
                       double nff = 0.0, double eeff = 0.0, double se = 0.0, double ncor = 0.0) :
        E_kin(kin), E_nn(nn), E_en(en), E_ee(ee),
        E_x(x), E_xc(xc), E_next(next), E_eext(eext),
        Er_tot(rt), Er_nuc(rn), Er_el(re),
        E_nff(nff), E_eeff(eeff), E_se(se), E_ncor(ncor) {
            E_nuc = E_nn + E_next + Er_nuc  + E_nff;
            E_el = E_kin + E_en + E_ee + E_xc + E_x + E_eext + Er_el + E_eeff + E_se + E_ncor;
        }
